...
namespace WebSocketSharp.Tests
{
  ...
  public sealed class AssemblyIdentityTests
  {
    ...
    public void BuiltAssemblyKeepsExpectedReleaseMetadata ()
    {
      ...
      Assert.That (fileVersion.Version, Is.EqualTo ("1.1.0.0"));
      Assert.That (informationalVersion.InformationalVersion, Is.EqualTo ("1.1.0.0"));
      Assert.That (product.Product, Is.EqualTo ("websocket-sharp.dll"));
      Assert.That (fileInfo.FileVersion, Is.EqualTo ("1.1.0.0"));
      Assert.That (fileInfo.ProductVersion, Is.EqualTo ("1.1.0.0"));
